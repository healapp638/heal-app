...
router.post('/revenuecat_webhook', async (req: Request | any, res: Response) => {
    const controller = new UserSubscriptionController(req, res);
    const result: ApiResponse = await controller.revenueCatWebhook(req.body);
    return showOutput(res, result, result.code);
});

router.get('/subscription_status', verifyTokenUser, async (req: Request | any, res: Response) => {
    const controller = new UserSubscriptionController(req, res);
    const result: ApiResponse = await controller.checkSubscriptionStatus();
    return showOutput(res, result, result.code);
});

router.post('/sync_purchase', verifyTokenUser, async (req: Request | any, res: Response) => {
    const controller = new UserSubscriptionController(req, res);
    const result: ApiResponse = await controller.syncAfterPurchase();
    return showOutput(res, result, result.code);
});
